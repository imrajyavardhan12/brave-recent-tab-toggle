...
final class HostRuntime: @unchecked Sendable {
    ...
    init(environment: [String: String] = ProcessInfo.processInfo.environment) throws {
        ...
        let coordinationDirectory = environment["RTT_COORDINATION_ROOT"]
            .map { URL(fileURLWithPath: $0, isDirectory: true) }
            ?? defaultCacheDirectory
        let namespace = environment["RTT_EVENT_NAMESPACE"]
            ?? DistributedEventBus.defaultNamespace
        let targetBundleID = environment["RTT_TARGET_BUNDLE_ID"]
            ?? HotKeyPolicy.braveStableBundleID
        ...
        controller = HotKeyActivationController(
            registrar: registrar,
            policy: HotKeyPolicy(targetBundleID: targetBundleID)
        )
    }
