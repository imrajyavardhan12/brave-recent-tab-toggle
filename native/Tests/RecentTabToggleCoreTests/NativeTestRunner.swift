...
struct NativeTestRunner {
    static func main() throws {
        if CommandLine.arguments.contains("--broadcast-toggle") {
            let namespace = ProcessInfo.processInfo.environment["RTT_EVENT_NAMESPACE"]
        ...
        try messageIsFramedForChromiumNativeMessaging()
        print("✓ a message is framed for Chromium native messaging")
        try messageSinkWritesACompleteFrame()
        print("✓ a message sink writes a complete frame")
    ...
    static func messageIsFramedForChromiumNativeMessaging() throws {
        let frame = try NativeMessageWriter().frame(["type": "toggle"])
        let length = frame.prefix(4).withUnsafeBytes {
            UInt32(littleEndian: $0.loadUnaligned(as: UInt32.self))
        }
        try expect(Int(length) == frame.count - 4, "frame length prefix is incorrect")

        let payload = try JSONSerialization.jsonObject(with: frame.dropFirst(4)) as? [String: String]
    ...
    static func hotkeyIsOnlyActiveForBraveStable() throws {
        let policy = HotKeyPolicy()

        try expect(policy.shouldRegister(frontmostBundleID: "com.brave.Browser"), "Brave Stable should activate the hotkey")
        try expect(!policy.shouldRegister(frontmostBundleID: "com.brave.Browser.beta"), "Brave Beta should not activate the hotkey")
        try expect(!policy.shouldRegister(frontmostBundleID: "com.apple.Terminal"), "other applications should not activate the hotkey")
        try expect(!policy.shouldRegister(frontmostBundleID: nil), "a missing application should not activate the hotkey")
    }
    ...
    static func messageSinkWritesACompleteFrame() throws {
        ...
        let expected = try NativeMessageWriter().frame(["type": "toggle"])
        try expect(actual == expected, "message sink did not write the expected frame")
    }
}
